...
export async function updateTask(
	date: Date,
	taskId: string,
	patch: Partial<Pick<ActivityItem, "title" | "subtitle" | "completedMinutes" | "category">>,
): Promise<ActivityItem[]> {
	const tasks = await getTasksForDate(date);
	const updated = tasks.map((t) =>
...
async function getScheduledRaw(): Promise<ScheduledActivity[]> {
	...
	try {
		...
		const raw = await AsyncStorage.getItem(key);
		if (!raw) return [];
		const parsed = JSON.parse(raw) as (ScheduledActivity & { category?: string })[];
		if (!Array.isArray(parsed)) return [];
		return parsed.map((a) => {
			const date = a.date ?? "";
			const raw = a as ScheduledActivity & { deadline?: string; completed?: boolean };
			return {
				...a,
				category: (a.category ?? DEFAULT_CATEGORY) as ScheduledActivity["category"],
...
export async function getAllScheduledActivities(): Promise<ScheduledActivity[]> {
