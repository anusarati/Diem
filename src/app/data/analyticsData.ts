...
export async function buildActivityBreakdown(
	...
	return items.filter((i) => i.percent > 0).length > 0
		? items
...
export async function buildGoalTimeData(
	...
	for (const a of scheduledInRange) {
		plannedByCategory[a.category] =
			(plannedByCategory[a.category] ?? 0) + a.durationMinutes;
	}
	...
	for (const { tasks } of taskRanges) {
		for (const t of tasks) {
			...
			completedByCategory[cat] =
				(completedByCategory[cat] ?? 0) + t.completedMinutes;
		}
	}
	for (const a of scheduledInRange.filter((x) => x.completed)) {
		const mins = a.actualMinutesSpent ?? a.durationMinutes;
		completedByCategory[a.category] =
			(completedByCategory[a.category] ?? 0) + mins;
	}
...
function _parseMinutes(value: string): number {
