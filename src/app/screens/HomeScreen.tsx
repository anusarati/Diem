...
export function HomeScreen({ onNavigate }: Props) {
	...
	const [editChecklistTask, setEditChecklistTask] = useState<ActivityItem | null>(null);
	const [editScheduledActivity, setEditScheduledActivity] = useState<ScheduledActivity | null>(null);
	const [completingChecklist, setCompletingChecklist] = useState<ActivityItem | null>(null);
	const [completingScheduled, setCompletingScheduled] = useState<ScheduledActivity | null>(null);
	...
	const handleLogTimeChecklist = useCallback(
		(minutes: number | null) => {
			...
			toggleTaskCompleted(today(), completingChecklist.id, minutes != null ? { completedMinutes: minutes } : undefined).then(setActivities);
	...
	const logTimeModalVisible = completingChecklist != null || completingScheduled != null;
	...
	const handleLogTimeSelect = useCallback(
		...
		[completingChecklist, completingScheduled, handleLogTimeChecklist, handleLogTimeScheduled],
	);
				...
				>
					<View style={styles.activitiesSection}>
						...
						{loading ? (
							...
							<Text style={styles.muted}>No tasks yet. Tap Add task to add one.</Text>
						) : (
							combined.map((item, i) =>
								item.type === "checklist" ? (
									<ActivityRow
...
const styles = StyleSheet.create({
	...
	scroll: { flex: 1 },
	scrollContent: { paddingHorizontal: spacing.xl, paddingVertical: spacing.lg, paddingBottom: 48 },
