...
export function SettingsScreen({ onNavigate }: Props) {
	...
	const toggleNotifications = async () => {
		if (notificationsEnabled) {
			// Cannot revoke easily programmatically, just update state for demo
	...
	const testNotification = async () => {
		...
		await Notifications.scheduleNotificationAsync({
			...
			trigger: null, // metrics: immediate
		});
				...
				<View style={styles.section}>
					...
					<View style={styles.logContainer}>
						{debugLog.length === 0 ? (
							...
							debugLog.map((log, i) => (
								<Text
									// biome-ignore lint/suspicious/noArrayIndexKey: debug logs
									key={`log-${i}`}
...
const styles = StyleSheet.create({
	...
	logText: {
		color: colors.mint, // Matrix style
