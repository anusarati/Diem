...
export function GoalTimeRow({ goal, last }: Props) {
  const pct = goal.targetMinutes > 0
    ...
    <View style={[styles.row, !last && styles.rowBorder]}>
      <View style={styles.header}>
        ...
        <View style={styles.badges}>
          <Text style={styles.value}>{formatMinutes(goal.completedMinutes)}</Text>
      ...
      <View style={styles.projectedRow}>
        ...
        <Text style={styles.projectedValue}>{formatMinutes(goal.projectedMinutes)}</Text>
      </View>
      <View style={styles.track}>
        <View
          style={[
            ...
            {
              width: `${pct}%`,
              backgroundColor: goal.onTrack ? colors.mintDark : colors.peachDark,
            },
          ]}
        />
      </View>
...
const styles = StyleSheet.create({
  ...
  projectedLabel: { fontSize: 11, fontWeight: '600', color: colors.slate400, textTransform: 'uppercase' },
  projectedValue: { fontSize: 12, fontWeight: '700', color: colors.lavenderDark },
  track: {
    height: 6,
    width: '100%',
