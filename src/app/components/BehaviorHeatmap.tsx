...
export function BehaviorHeatmap({ data, columnCount = 12 }: Props) {
	...
	const _totalH = 20 + rowCount * cellSize;
		...
		<View style={[styles.wrap, { width: totalW }]}>
			<View style={styles.gridWrap}>
				...
				<View style={styles.grid}>
					{/* biome-ignore-start lint/suspicious/noArrayIndexKey: fixed 7×12 grid, no item identity */}
					{data.map((row, i) => (
						<View key={`heatmap-row-${i}`} style={styles.row}>
							{row.map((val, j) => (
								<View
			...
			<Text style={styles.caption}>
				Activity intensity by day & time (behavior model)
			</Text>
		</View>
	);
}
